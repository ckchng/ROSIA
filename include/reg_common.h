...
#define MIN(a,b) ((a) < (b) ? a : b)
    ...
    struct Matrix1X
    {
        ...
        Matrix1X(size_t n): n(n)
        {
//            mxAssert(n>0,"");
            x = (double *)malloc(n*1*sizeof(double));
        }

        Matrix1X(const Matrix1X& other) : n(other.n)
        {
//            mxAssert(n>0,"");
            x = (double *)malloc(n*1*sizeof(double));
            std::copy(other.x, other.x+1*n, x);
...
//            mxAssert(newSize>=0,"size must be >=0");
        ...
        //Copy assignment
        Matrix1X &operator=(const Matrix1X &other)
        {
            n=other.n;
            x = (double *)malloc(n*1*sizeof(double));
            //memcpy(x,other.x,n*3*sizeof(double));
            std::copy(other.x, other.x + n*1, x);
            return *this;
        }
...
//    inline Vector3 multiply(const Matrix3 &M, const std::vector<double> v)
//    {
//        double x,y,z;
//        const double *m = M.m;
//        x = v[0]*m[0] + v[1]*m[3] + v[2]*m[6];
...
//            dgemm(chn, chn, &m, &m, &m, &alpha, this->x, &m, tform.x, &m, &beta, resp.x, &m);
            cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                        m, m, m, alpha, this->x, m, tform.x, m, beta, resp.x, m);
...
//                 "axis must be an unit-vector");
    ...
    inline
    void rTranpose(Matrix3 &R_T, const Matrix3 &R)
    {
        R_T.m[0] = R.m[0];
        R_T.m[3] = R.m[1];
        R_T.m[6] = R.m[2];
